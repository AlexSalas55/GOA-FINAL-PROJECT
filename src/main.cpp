...
void buildSceneCornellBox2(Camera*& cam, Film*& film, Scene myScene)
{
    ...
    // Luz de �rea: rect�ngulo grande en el techo, mirando hacia abajo
    ...
    // Definici�n de bolas (x,z). Se ajustar�n para evitar solapes.
    ...
    std::vector<Ball> balls = {
        ...
        {0.9, Vector3D(-1.60, 0.0, 5.60), glossyRed},     // roja peque�a
        ...
        {0.4, Vector3D( 3.40, 0.0, 6.20), glossyYellow},  // amarilla peque�a
    };

    // Relajaci�n en planta (x,z) para evitar solapes.
    // Para esferas sobre el mismo plano, distancia horizontal m�nima = 2*sqrt(r1*r2).
    const double margin = 0.02; // separador m�nimo
    ...
    // Create geometry in the scene (centers resting on floor)
    int sphereIndex = 0;
...
int main()
{
    ...
    //Lab 2 - Task 4.2.2
    Shader *areadirectshader = new AreaDirect(bgColor, 256);
